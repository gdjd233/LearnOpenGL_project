...
int work_03_01(int avgc, char* argv[])
{
	// OpenGL
	...
	// 创建着色器类
	// (顶点着色器路径，片段着色器路径）
	Shader ourShader("./exercises/03_shader/shader1/vertex.vs", "./exercises/03_shader/shader1/fragment.fs");

	// 创建三角形三个顶点坐标，把三角形的三个角分别指定为红色、绿色和蓝色
	float vertices[] = {
		// 位置              // 颜色
	...
	第五个参数：步长，这里有两个vec3数据长度是6 * sizeof(float)，当数据是紧密相连的也可以设置成0让OpenGL自己计算
