...
用你的函数实现来替换GLM的LookAt函数，看看它是否还能一样地工作
*/
...
int work_07_02(int avgc, char* argv[])
{
	...
	// 创建着色器类
	Shader ourShader("./exercises/07_camera/shader/vertex.vs", "./exercises/07_camera/shader/fragment.fs");
	...
	*/
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// 使用自定义Look At函数
		view = calculate_lookAt_matrix(camera.Position, camera.Position + camera.Front, camera.Up);
		// 创建投影矩阵
		glm::mat4 projection;
...
static glm::mat4 calculate_lookAt_matrix(glm::vec3 position, glm::vec3 target, glm::vec3 worldUp)
{
	glm::vec3 arr[3];
	// 计算z方向（摄像机方向）
	arr[2] = glm::normalize(position - target);
	// 计算x方向（右方向）
	arr[0] = glm::normalize(glm::cross(glm::normalize(worldUp), arr[2]));
	// 计算y方向（上方向）
	arr[1] = glm::cross(arr[2], arr[0]);
