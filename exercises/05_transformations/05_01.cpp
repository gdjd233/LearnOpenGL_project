...
看看发生了什么，试着想想为什么会发生这样的事情

答：图片绕屏幕中心点逆时针旋转
是因为实际变换时先位移变换后旋转变换，相当于位移向量也在旋转。乘后的变换矩阵的第四列并不是0.5 -0.5 0.0 1，而是1/2*cosθ+1/2*sinθ 1/2*sinθ-1/2cosθ 0.0 1。因此会造成绕（0,0）点旋转的效果
*/

int work_05_01(int avgc, char* argv[])
{
	// OpenGL
	...
	// 创建着色器类
	Shader ourShader("./exercises/05_transformations/shader/vertex.vs", "./exercises/05_transformations/shader/fragment.fs");
	...
	*/
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// 把矩阵数据发送给shader
		// （地址， 发送多少个， 是否对矩阵转置（交换矩阵的行和列），矩阵数据（要转换一下））
