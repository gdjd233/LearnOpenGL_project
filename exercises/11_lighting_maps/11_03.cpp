...
int work_11_03(int avgc, char* argv[])
{
	...
	// 然而，这些操作在每个系统上都是不一样的，OpenGL有意将这些操作抽象(Abstract)出去。这意味着我们不得不自己处理创建窗口，定义OpenGL上下文以及处理用户输入。
	...
	// 在一个 OpenGL 应用中，可能会有多个窗口或者多个上下文。例如，一个游戏可能有一个主窗口用于渲染场景，另一个窗口用于渲染 UI 或调试信息。
	// 每个窗口都有可能有自己的 OpenGL 上下文，而 OpenGL 本身并不知道当前活跃的上下文。
	...
	// 创建着色器类
	// 与光照计算相关的着色器类
	Shader lightingShader("./exercises/11_lighting_maps/shader3/cube.vs", "./exercises/11_lighting_maps/shader3/cube.fs");
	// 与“发光立方体”或“立方体光源”相关的着色器
	Shader lightCubeShader("./exercises/11_lighting_maps/shader3/light_cube.vs", "./exercises/11_lighting_maps/shader3/light_cube.fs");
	...
	*/
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// 使用inverse和transpose函数自己生成这个法线矩阵，这两个函数对所有类型矩阵都有效。注意我们还要把被处理过的矩阵强制转换为3×3矩阵，来保证它失去了位移属性以及能够乘以vec3的法向量。
